...
- **finance-app** - Bill and subscription tracker app
...
```bash
tmux new -s clawd-game -d 'cd ~/repos/game-project && clawd --perpetual "Continuously improve code quality: fix lint errors, add missing tests, refactor complex functions, and address TODO comments"'
```
You can check progress with `tmux attach -t clawd-game` and stop with Ctrl+C.
