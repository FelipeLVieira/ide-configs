...
If you see "expiring (Xh)" in doctor output or authentication errors:
...
This creates a 1-year OAuth token stored in `~/.clawdbot/agents/main/agent/auth-profiles.json`.

**Important differences:**
- **Session tokens**: Short-lived (hours), auto-refresh, can fail
- **Long-lived tokens**: 365 days, stable, recommended for production

**Symptoms of token expiration:**
- "AI service temporarily overloaded" errors
- "OAuth token refresh failed" in logs
...
cat ~/.clawdbot/agents/main/agent/auth-profiles.json | grep expires
```

**Prevention:**
Set up a reminder to run `claude setup-token` every 6 months to avoid expiration issues.
...
If you see `Context overflow: prompt too large for the model`:

**Common Causes:**
1. **Dual-instance conflict**: Multiple machines using same Telegram bot token
2. **Large workspace context**: Too many files in workspace directory
3. **Long conversation history**: Extended conversation without compaction

**Solution 1: Check for dual-instance conflict** (MOST COMMON)

If you have multiple machines (e.g., MacBook + Mac Mini) running clawdbot with the same Telegram bot token:
- Only ONE machine should have `channels.telegram.enabled: true`
- Other machines should have `channels.telegram.enabled: false` (backup/standby)
- Both responding to same message doubles the context load

**Solution 2: Use compaction mode**

Clawdbot 2026.1.24+ has automatic compaction on overflow:
```json
{
  "agents": {
    "defaults": {
...
**Solution 3: Manual compaction**

Users can send `/compact` to bot on Telegram to manually summarize older messages. This preserves more context intelligently.
...
**Solution 5: Reduce workspace context**

Move large files out of workspace or use a minimal workspace for bot operations.
...
```bash
clawdbot config get agents.defaults.compaction.mode
...
**Problem:** If both machines have `telegram.enabled: true`, they will BOTH try to respond to every message:
- Doubles the context load (two agents processing same conversation)
- Causes "Context overflow" errors
- Creates duplicate responses
- Wastes API quota

**Solution:** Enable Telegram on PRIMARY machine only, disable on BACKUP machines.

### Switching Primary/Backup

To switch which machine is primary:

```bash
# On old primary (MacBook) - disable Telegram
clawdbot config set channels.telegram.enabled false

# On new primary (Mac Mini) - enable Telegram
clawdbot config set channels.telegram.enabled true
```

Both gateways will restart automatically on config change.

### Verification

Check which machine is responding:
```bash
# On each machine, check Telegram status
clawdbot status | grep telegram

# Should see:
# Primary: telegram - connected (@your_bot_username)
# Backup: telegram - disabled
```

### Use Cases

- **Primary**: Development machine, always-on desktop
- **Backup**: Laptop for travel, server for redundancy

Both can use the same workspace (via network share/sync) but only one should handle Telegram messages at a time.

## Bot Self-Awareness & Context Management

Configure your bot to proactively manage context and prevent overflow errors.

### Add Context Instructions to IDENTITY.md

Edit `~/clawd/IDENTITY.md` (or your workspace IDENTITY.md):
...
1. Bot reads `IDENTITY.md` at session start
2. Bot becomes aware of context management capabilities
3. Bot proactively suggests `/compact` when conversations get long
4. Prevents overflow errors before they happen

### Verification

Ask your bot: "Do you know how to handle context overflow?"

Expected response: Bot should mention `/compact` command and automatic safeguard mode.
